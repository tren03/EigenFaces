...
# compute the LDA (Fisherfaces) representation of the data, then
# project the training data onto the Fisherfaces subspace
print("[INFO] creating Fisherfaces...")
lda = LDA()
start = time.time()
trainX = lda.fit_transform(trainX, trainY)
end = time.time()
print("[INFO] computing Fisherfaces took {:.4f} seconds".format(end - start))
...
if args["visualize"] > 0:
	...
	for (i, component) in enumerate(lda.scalings_[:16]):
...
predictions = model.predict(lda.transform(testX))
print("*** SVC REPORT ***")
print(classification_report(testY, predictions, target_names=le.classes_))


# Train a Random Forest classifier on the Fisherfaces representation
print("[INFO] training Random Forest classifier...")
random_forest_model = RandomForestClassifier(n_estimators=100, random_state=42)
random_forest_model.fit(trainX, trainY)
# Evaluate the model
print("[INFO] evaluating Random Forest model...")
predictions_rf = random_forest_model.predict(lda.transform(testX))
print("*** RANDOM FOREST REPORT ***")
print(classification_report(testY, predictions_rf, target_names=le.classes_))

# Train a k-Nearest Neighbors classifier on the Fisherfaces representation
...
print("*** KNN REPORT ***")
predictions_knn = knn_model.predict(lda.transform(testX))

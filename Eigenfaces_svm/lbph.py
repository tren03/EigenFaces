...
#     for j in range(0, s2 - 8, 8):
#         for k in range(0, s3 - 8, 8):
#             block = img_por[j:j + 8, k:k + 8]
#             hist, _ = numpy.histogram(block, bins=numpy.arange(257), range=(0, 256))
#             histogram += hist
#     return histogram
...
def localBinaryPattern(images):
    ...
    for i in range(s1):
        ...
        for j in range(s2-window-1):
            for k in range(s3-window-1):
                temp2 = images[i, j:j+window, k:k+window]
                temp1 = int(getPattern(temp2))
                images_processed[i, j, k] = temp1
    return images_processed
...
def predict_lbph_multi(input_images, recognizer, labels):
    (s3, s1, s2) = input_images.shape
    (d1, d2) = recognizer.shape
    predictions = numpy.zeros((s3))
    minval_confidence = numpy.zeros((s3))
    input_histogramed = train_lbph(input_images)
    for j in range(s3):
        (minval, index, distance) = (float('inf'), 0, 0)
        for i in range(d1):
            distance = numpy.linalg.norm(recognizer[i, :] - input_histogramed[j, :])
        ...
        predictions[j] = labels[index]
        minval_confidence[j] = minval
    confidence = calculate_confidence(minval)
